    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
      {formattedNotes.map((note) => (
        <Link key={note.note_id} href={`/notes/${note.note_id}`}>
          <div className="border border-border rounded-xl p-4 bg-background shadow-sm hover:shadow-md transition-all duration-200 cursor-pointer h-full flex flex-col justify-between hover:brightness-[200%]">
            <h2 className="text-lg font-semibold mb-2 line-clamp-2">{note.title}</h2>
